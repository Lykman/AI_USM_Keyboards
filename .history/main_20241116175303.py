...
while True:
    ...
    text = typed_text
    try:
        text_width, text_height = font.getbbox(text)[2], font.getbbox(text)[3]  # Используем getbbox для получения размера
    except AttributeError:
        # Если getbbox недоступен, используем альтернативный метод
