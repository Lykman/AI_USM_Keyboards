...
# Для ограничения FPS и предотвращения многократных срабатываний жестов
last_gesture_time = 0
gesture_cooldown = 1  # секунды
...
while True:
    ...
    if hands:
        ...
        # Используем GestureControl для определения жеста
        gesture = gesture_control.detect_gesture(lmList)
        print(f"Detected gesture: {gesture}")

        # Проверка, прошло ли достаточно времени с последнего жеста
        if (current_time - last_gesture_time) > gesture_cooldown:
            if gesture == 'SWIPE_LEFT':
                print("SWIPE_LEFT gesture detected: Switching Theme")
                # Смена темы
                ...
                print("SWIPE_RIGHT gesture detected: Switching Language")
                # Смена языка
            ...
            elif gesture == 'TWO_FINGERS_TOUCH':
                print("TWO_FINGERS_TOUCH gesture detected: Toggle Shift")
                # Переключение состояния Shift
