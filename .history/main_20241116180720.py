...
from modules.gesture_control import GestureControl  # Импорт класса GestureControl
import os
...
if os.path.exists(logo_path):
    ...
    if logo is not None:
        ...
        print(f"Logo loaded with size: {logo.shape}")
    else:
...
# Отображение клавиатуры включено по умолчанию
...
while True:
    ...
    if new_frame_width != frame_width or new_frame_height != frame_height:
        ...
        if logo is not None:
            ...
            logo_height = int(logo.shape[0] * scaling_factor)
            logo = cv2.resize(logo, (logo_width, logo_height), interpolation=cv2.INTER_AREA)
            print(f"Logo resized to: {logo.shape}")
    ...
    # Получение списка лендмарков для GestureControl
    if hands:
    ...
    # Действия на основе жеста
    if gesture == 'PINCH':
        # Переключение темы
        print("PINCH gesture detected: Switching theme")
        ...
        virtual_keyboard = Keyboard(language=current_language, theme=current_theme, frame_width=frame_width, frame_height=frame_height)
        cv2.waitKey(300)  # Задержка для предотвращения множественных срабатываний
    elif gesture == 'POINT_UP':
        # Переключение языка
        print("POINT_UP gesture detected: Switching language")
        current_language = 'RU' if current_language == 'EN' else 'EN'
        virtual_keyboard = Keyboard(language=current_language, theme=current_theme, frame_width=frame_width, frame_height=frame_height)
        cv2.waitKey(300)
    elif gesture == 'FIST':
        # Скрытие/показ клавиатуры
        keyboard_visible = not keyboard_visible
        print(f"Keyboard {'shown' if keyboard_visible else 'hidden'}")
        cv2.waitKey(300)
    ...
    try:
        ...
        # Если getbbox отсутствует, используем textsize
    ...
    # Рисуем виртуальную клавиатуру, если она видна
    if keyboard_visible:
        virtual_keyboard.draw(img)

    if hands and keyboard_visible:
        lmList = hands[0]['lmList']
        ...
        # Если нет рук в кадре или клавиатура скрыта, просто рисуем кнопки (если клавиатура видима)
        if keyboard_visible:
            virtual_keyboard.draw(img)
