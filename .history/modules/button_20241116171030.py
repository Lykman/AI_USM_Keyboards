...
class Button:
    ...
    def draw(self, img):
        ...
        # Преобразуем цвета из списков в кортежи и в int
        button_color = tuple(int(c) for c in self.theme['button_color'])
        border_color = tuple(int(c) for c in self.theme['border_color'])
        ...
        draw_pil.text((text_x, text_y), self.text, font=self.font, fill=text_color)
    ...
    def draw_hover(self, img):
        ...
        # Преобразуем цвета из списков в кортежи и в int
        hover_color = tuple(int(c) for c in self.theme['hover_color'])
        border_color = tuple(int(c) for c in self.theme['border_color'])

        # Рисуем кнопку с эффектом наведения
        cv2.rectangle(img, (x, y), (x + w, y + h), hover_color, -1)
        cv2.rectangle(img, (x, y), (x + w, y + h), border_color, 2)
    ...
    def draw_highlight(self, img):
        ...
        # Преобразуем цвета из списков в кортежи и в int
        pressed_color = tuple(int(c) for c in self.theme['pressed_color'])
        border_color = tuple(int(c) for c in self.theme['border_color'])

        # Рисуем кнопку с эффектом нажатия
        cv2.rectangle(img, (x, y), (x + w, y + h), pressed_color, -1)
        cv2.rectangle(img, (x, y), (x + w, y + h), border_color, 2)
        ...
        text_color = tuple(int(c) for c in self.theme['text_pressed_color'])
        draw_pil.text((text_x, text_y), self.text, font=self.font, fill=text_color)
